 ...
 *
 * Updated for the 2.6 kernel by Ajit Prem (Motorola)
 *
...
extern int gt64260_i2cMacGet(char *, int);
extern void flush_dcache_range(unsigned long, unsigned long);
extern void invalidate_dcache_range(unsigned long, unsigned long);
...
u32 gt64260_base = CONFIG_MV64X60_NEW_BASE;
...
gt64260_eth_init(int port_num)
{
	...
	dev = alloc_etherdev(sizeof (gt_eth_priv));
	if (!dev) {
	...
	case 0:
		if (!gt64260_eth0_initialized) {
			if (gt64260_i2cMacGet(dev->dev_addr, 0) == -1) {
				printk("%s: Couldn't Read Srom MAC address\n",
				       dev->name);
 ...
 */
static irqreturn_t
gt64260_eth_int_handler(s32 irq, void *dev_id)
{
	gt_eth_priv *priv;
	struct net_device *dev;
	u32 eth_int_cause;
...
#ifdef	CONFIG_GT64260_ETH_0
	if (gt64260_eth_init(0)) {
                printk(KERN_ERR "Error registering GT64260 ethernet port 0\n");
	}
#endif
#ifdef	CONFIG_GT64260_ETH_1

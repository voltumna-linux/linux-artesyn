	...
	struct vme_dma_attr *src, struct vme_dma_attr *dest, size_t count)
{
	...
	entry->dma_handle = dma_map_single(dev, &entry->descriptor,
					   ...
					   DMA_TO_DEVICE);
	if (dma_mapping_error(dev, entry->dma_handle)) {
		retval = -ENOMEM;
		list_del(&entry->list);
		goto err_align;
	}
...
static int ca91cx42_dma_list_empty(struct vme_dma_list *list)
{
	...
	list_for_each_safe(pos, temp, &list->entries) {
		...
		dma_unmap_single(dev, entry->dma_handle,
				 sizeof(entry->descriptor), DMA_TO_DEVICE);
		kfree(entry);
	}

 ...
 *
 *  Modified by Ajit Prem (ajit.prem@motorola.com) 2001 to support dual MPICs
 *
...
#if defined(CONFIG_MCP805) || defined(CONFIG_MCP820) || defined(CONFIG_PRPMC800) || defined(CONFIG_85xx)
#define OPENPIC_BIG_ENDIAN
#endif

void __iomem *OpenPIC_Addr;
void __iomem *OpenPIC2_Addr;
static volatile struct OpenPIC __iomem *OpenPIC = NULL;
static volatile struct OpenPIC __iomem *OpenPIC2 = NULL;

extern int ppc_spurious_interrupts;

/*
 * We define OpenPIC_InitSenses table thus:
 * bit 0x1: sense, 0 for edge and 1 for level.
 * bit 0x2: polarity, 0 for negative, 1 for positive.
 */
u_int OpenPIC_NumInitSenses __initdata = 0;
u_int OpenPIC2_NumInitSenses __initdata = 0;

u_char *OpenPIC_InitSenses __initdata = NULL;
u_char *OpenPIC2_InitSenses __initdata = NULL;
extern int use_of_interrupt_tree;

static int openpic2_get_irq(void);
...
int (*cascaded[NR_IRQS]) (void);

static u_int NumProcessors;
static u_int NumSources;
static u_int NumSources2;
int open_pic_irq_offset;
int open_pic2_irq_offset;

static volatile OpenPIC_Source __iomem *ISR[NR_IRQS];
static volatile OpenPIC_Source __iomem *ISR2[NR_IRQS];
static int openpic_cascade_irq = -1;
static int (*openpic_cascade_fn) (void);

/* Global Operations */
static void openpic_disable_8259_pass_through(struct OpenPIC *OpenPIC);
static void openpic_set_priority(struct OpenPIC *OpenPIC, u_int pri);
static void openpic_set_spurious(struct OpenPIC *OpenPIC, u_int vector);
...
static void openpic_initipi(struct OpenPIC *OpenPIC, u_int ipi,
			    u_int pri, u_int vector);
static irqreturn_t openpic_ipi_action(int cpl, void *dev_id);
#endif

/* Timer Interrupts */
static void openpic_inittimer(struct OpenPIC *OpenPIC, u_int timer,
			      u_int pri, u_int vector);
static void openpic_maptimer(struct OpenPIC *OpenPIC, u_int timer, cpumask_t cpumask);
...
static void openpic_initirq(struct OpenPIC *OpenPIC, u_int irq,
			    u_int pri, u_int vector, int polarity,
			    int is_level);
static void openpic_mapirq(struct OpenPIC *OpenPIC, u_int irq,
			   cpumask_t cpumask, cpumask_t keepmask);

/*
 * These functions are not used but the code is kept here
 * for completeness and future reference.
 */
static u_int openpic_get_priority(struct OpenPIC *OpenPIC);
#ifdef notused
static void openpic_enable_8259_pass_through(struct OpenPIC *OpenPIC);
static u_int openpic_get_spurious(struct OpenPIC *OpenPIC);
static void openpic_set_sense(struct OpenPIC *OpenPIC, u_int irq,
			      int sense);
#endif				/* notused */
 ...
 * anyway --Troy
 */
#define check_pic1_arg_irq(irq) \
    if (irq < open_pic_irq_offset || irq >= NumSources + open_pic_irq_offset \
		|| ISR[irq - open_pic_irq_offset] == 0) { \
      printk("open_pic.c:%d: illegal irq %d\n", __LINE__, irq); \
      dump_stack(); }
#define check_pic2_arg_irq(irq) \
    if (irq < open_pic2_irq_offset || irq >= NumSources2 + \
					open_pic2_irq_offset \
		|| ISR2[irq - open_pic2_irq_offset] == 0) { \
      printk("open_pic.c:%d: illegal irq %d\n", __LINE__, irq); \
      dump_stack(); }
...
inline void openpic_writefield_IPI(volatile u_int __iomem * addr, u_int mask,
				   u_int field)
{
...
static inline void openpic_clearfield_IPI(volatile u_int __iomem * addr,
					  u_int mask)
{
	openpic_writefield_IPI(addr, mask, 0);
}

static inline void openpic_setfield_IPI(volatile u_int __iomem * addr,
					u_int mask)
{
	openpic_writefield_IPI(addr, mask, mask);
}

static void openpic_safe_writefield_IPI(volatile u_int __iomem * addr,
					u_int mask, u_int field)
{
...
void __init openpic_set_sources(int first_irq, int num_irqs,
				void __iomem * first_ISR)
{
	...
	if (src == 0)
		src =
		    &((struct OpenPIC __iomem *)OpenPIC_Addr)->Source[first_irq];
	for (i = first_irq; i < last_irq; ++i, ++src)
		ISR[i] = src;
}

void __init openpic2_set_sources(int first_irq, int num_irqs,
				void __iomem * first_ISR)
{
	volatile OpenPIC_Source __iomem *src = first_ISR;
	int i, last_irq;
	...
	last_irq = first_irq + num_irqs;
	if (last_irq > NumSources2)
		NumSources2 = last_irq;
	if (src == 0)
		src =
		    &((struct OpenPIC __iomem *)OpenPIC2_Addr)->Source[first_irq];
	for (i = first_irq; i < last_irq; ++i, ++src)
		ISR2[i] = src;
}

#if 0
static irqreturn_t openpic_cascade_action(int irq, void *dev_id)
{
        int interrupt;

        interrupt = openpic2_get_irq();
        if (interrupt >= 0) {
                __do_IRQ(interrupt);
	}
        return IRQ_HANDLED;
...
//        .handler = openpic_cascade_action,
	.handler = no_action,
        .flags = IRQF_DISABLED,
        .mask = CPU_MASK_NONE,
        .name = "OPENPIC Cascade",
};
 ...
 * We support one cascaded OpenPIC.
 */

void __init openpic2_init(openpic_def * def)
{
	u_int t;
	const char *version;
	...
	if (def->OpenPIC_Addr) {	/* Set in the parameter block ? */
		OpenPIC2 = def->OpenPIC_Addr;	/* Use this one */
	} else {
		if (OpenPIC2_Addr) {
			OpenPIC2 = OpenPIC2_Addr;	/* Use that */
		} else {
	...
	open_pic2_irq_offset = def->IRQdef->Vector;

	if (ppc_md.progress)
		ppc_md.progress("openpic enter", 0x122);

	t = openpic_read(&OpenPIC2->Global.Feature_Reporting0);
	...
	NumSources2 = ((t & OPENPIC_FEATURE_LAST_SOURCE_MASK) >>
		       OPENPIC_FEATURE_LAST_SOURCE_SHIFT) + 1;

	if (OpenPIC2_NumInitSenses > NumSources2)
		NumSources2 = OpenPIC2_NumInitSenses;

	printk("OpenPIC2 Version %s (%d CPUs and %d IRQ sources) at %p\n",
	       version, NumProcessors, NumSources2, OpenPIC2);
	...
	openpic_set_priority((struct OpenPIC __iomem *)OpenPIC2, 0xf);
	...
	while (irq_def->PICIrq >= 0) {
		...
		if (ISR2[irq_def->PICIrq] == 0)
			continue;

		if (ppc_md.progress)
			ppc_md.progress("openpic disable", 0x3bd);

		/* the bootloader may have left it enabled (bad !) */
		openpic_disable_irq(irq_def->Vector);

		if (irq_def->IrqSense == OP_IRQ_LEVEL)
			irq_desc[irq_def->Vector].status = IRQ_LEVEL;

		/* Enabled */
		openpic_initirq((struct OpenPIC __iomem *)OpenPIC2,
				irq_def->PICIrq,
				irq_def->Priority,
				irq_def->Vector,
				irq_def->IrqPolarity, 
		...
		/* Processor 0 */
		openpic_mapirq((struct OpenPIC __iomem *)OpenPIC2, 
				irq_def->PICIrq,
				CPU_MASK_CPU0,
				CPU_MASK_NONE);

		/* Init descriptor */
		irq_desc[irq_def->Vector].chip = &open_pic;
	...
	openpic_set_spurious((struct OpenPIC __iomem *)OpenPIC2,
			     OPENPIC_VEC_SPURIOUS + open_pic2_irq_offset);

	openpic_disable_8259_pass_through((struct OpenPIC __iomem *)OpenPIC2);
#ifdef CONFIG_EPIC_SERIAL_MODE
	openpic_eicr_set_clk(7);	/* Slowest value until we know better */
	openpic_enable_sie();
#endif
	openpic_set_priority((struct OpenPIC __iomem *)OpenPIC2, 0);
...
//                    OPENPIC_FEATURE_LAST_SOURCE_SHIFT) + 1;
	...
	for (i = 0; i < OPENPIC_NUM_TIMERS; i++) {
		/* Disabled, Priority 0 */
		openpic_inittimer((struct OpenPIC __iomem *)OpenPIC, i, 0,
				  OPENPIC_VEC_TIMER + i + open_pic_irq_offset);
		/* No processor */
		openpic_maptimer((struct OpenPIC __iomem *)OpenPIC, i,
				 CPU_MASK_NONE);
	}
	...
	for (i = 0; i < OPENPIC_NUM_IPI; i++) {
		...
		openpic_initipi((struct OpenPIC __iomem *)OpenPIC, i,
				OPENPIC_PRIORITY_IPI_BASE + i,
				OPENPIC_VEC_IPI + i + open_pic_irq_offset);
		/* IPIs are per-CPU */
		irq_desc[OPENPIC_VEC_IPI + i + open_pic_irq_offset].status |=
		    IRQ_PER_CPU;
		irq_desc[OPENPIC_VEC_IPI + i + open_pic_irq_offset].handler =
		    &open_pic_ipi;
	}
#endif
	...
	openpic_set_priority((struct OpenPIC __iomem *)OpenPIC, 0xf);
	...
	while (irq_def->PICIrq >= 0) {
		...
		/* Enabled */
		openpic_initirq((struct OpenPIC __iomem *)OpenPIC,
				irq_def->PICIrq,
				irq_def->Priority,
				irq_def->Vector,
				irq_def->IrqPolarity, 
		...
		/* Processor 0 */
		openpic_mapirq((struct OpenPIC __iomem *)OpenPIC,
				irq_def->PICIrq, 
				CPU_MASK_CPU0, 
				CPU_MASK_NONE);
	...
	openpic_set_spurious((struct OpenPIC __iomem *)OpenPIC,
			     OPENPIC_VEC_SPURIOUS + open_pic_irq_offset);

	openpic_disable_8259_pass_through((struct OpenPIC __iomem *)OpenPIC);
#ifdef CONFIG_EPIC_SERIAL_MODE
	openpic_eicr_set_clk(7);/* Slowest value until we know better */
	openpic_enable_sie();
#endif
	openpic_set_priority((struct OpenPIC __iomem *)OpenPIC, 0);
...
#ifdef notused
static void openpic_enable_8259_pass_through((struct OpenPIC __iomem *)OpenPIC)
{
...
u_int openpic2_irq(void)
{
	...
	CHECK_THIS_CPU;
	vec = openpic_readfield(&OpenPIC2->THIS_CPU.Interrupt_Acknowledge,
				OPENPIC_VECTOR_MASK);
	return vec;
}
...
void openpic2_eoi(void)
{
	...
	CHECK_THIS_CPU;
	openpic_write(&OpenPIC2->THIS_CPU.EOI, 0);
	/* Handle PCI write posting */
	(void)openpic_read(&OpenPIC2->THIS_CPU.EOI);
}
...
static void openpic_set_spurious(struct OpenPIC *OpenPIC, u_int vec)
{
	check_arg_vec(vec);
	openpic_writefield(&OpenPIC->Global.Spurious_Vector,
			   OPENPIC_VECTOR_MASK, vec);
}
 ...
 */
static inline u32 physmask(cpumask_t cpumask)
{
	int i;
	u32 mask = CPU_MASK_NONE;
 ...
 */
static void __init openpic_initipi((struct OpenPIC __iomem *)OpenPIC, u_int ipi,
				   u_int pri, u_int vec)
{
	check_arg_ipi(ipi);
	check_arg_pri(pri);
...
void __init do_openpic_setup_cpu(void)
{
	...
	for (i = 0; i < NumSources; i++) {
		openpic_mapirq(OpenPIC, i, msk, CPU_MASK_ALL);
		openpic_mapirq(OpenPIC2, i, msk, CPU_MASK_ALL);
	}
 ...
 *  vec: the vector it will produce
 */
static void __init openpic_inittimer(struct OpenPIC *OpenPIC,
				     u_int timer, u_int pri, u_int vec)
{
 ...
 */
static void __init openpic_maptimer(struct OpenPIC *OpenPIC, u_int timer,
				    cpumask_t cpumask)
{
 ...
 */
void __init
openpic2_set_irq_priority(u_int irq, u_int pri)
{
	check_pic2_arg_irq(irq);
	openpic_safe_writefield(&ISR2[irq - open_pic2_irq_offset]->Vector_Priority, 
...
{
	if (irq >= open_pic2_irq_offset) {
		openpic2_set_irq_priority(irq, pri);
	} else {
		check_pic1_arg_irq(irq);
 ...
 */
void __init openpic2_init_nmi_irq(u_int irq)
{
	check_pic2_arg_irq(irq);
	openpic_set_irq_priority(irq, OPENPIC_PRIORITY_NMI);
}

void __init openpic_init_nmi_irq( u_int irq)
{
	if (irq >= open_pic2_irq_offset) {
		openpic2_init_nmi_irq(irq);
	} else {
		check_pic1_arg_irq(irq);
		openpic_set_irq_priority(irq, OPENPIC_PRIORITY_NMI);
	}
...
void __init
openpic_hookup_cascade(u_int irq, char *name, int (*cascade_fn)(void))
{
...
static void openpic2_enable_irq(u_int irq)
{
	...
	check_pic2_arg_irq(irq);
	picirq = irq - open_pic2_irq_offset;
	openpic_clearfield(&OpenPIC2->Source[picirq].Vector_Priority, OPENPIC_MASK);
	/* make sure mask gets to controller before we return to user */
	do {
		mb();		/* sync is probably useless here */
	} while (openpic_readfield(&OpenPIC2->Source[picirq].Vector_Priority, OPENPIC_MASK));
}
...
void openpic_enable_irq(u_int irq)
{
	...
	if (irq >= open_pic2_irq_offset) {
		openpic2_enable_irq(irq);
	} else {
		check_pic1_arg_irq(irq);
		picirq = irq - open_pic_irq_offset;
		vpp = &ISR[picirq]->Vector_Priority;
...
static void openpic2_disable_irq(u_int irq)
{
	...
	check_pic2_arg_irq(irq);
	picirq = irq - open_pic2_irq_offset;
	openpic_setfield(&OpenPIC2->Source[picirq].Vector_Priority, OPENPIC_MASK);
	/* make sure mask gets to controller before we return to user */
	do {
		mb();		/* sync is probably useless here */
		vp = openpic_readfield(&OpenPIC2->Source[picirq].
				       Vector_Priority,
...
void openpic_enable_ipi(u_int irq)
{
	...
	check_arg_ipi(irq);
	openpic_clearfield_IPI(&OpenPIC->Global.IPI_Vector_Priority(irq),
			       OPENPIC_MASK);
...
void openpic_disable_ipi(u_int irq)
{
	irq -= (OPENPIC_VEC_IPI + open_pic_irq_offset);
	check_arg_ipi(irq);
	openpic_setfield_IPI(&OpenPIC->Global.IPI_Vector_Priority(irq),
			     OPENPIC_MASK);
}
#endif
 ...
 */
static void openpic_initirq(struct OpenPIC *OpenPIC, u_int irq, u_int pri,
			    u_int vec, int pol, int sense)
{
	openpic_safe_writefield(&OpenPIC->Source[irq].Vector_Priority,
				...
				((sense == OP_IRQ_LEVEL) ?
			       OPENPIC_SENSE_LEVEL : OPENPIC_SENSE_EDGE));
}

/*
 *  Map an interrupt source to one or more CPUs
 */
static void openpic_mapirq(struct OpenPIC *OpenPIC, u_int irq, cpumask_t physmask, cpumask_t keepmask)
{
#if 0
        if (ISR[irq] == 0)
                return;
        ...
        if (!cpus_empty(keepmask)) {
                cpumask_t irqdest = { .bits[0] = openpic_read(&OpenPIC->Source[irq].Destination) };
                cpus_and(irqdest, irqdest, keepmask);
                cpus_or(physmask, physmask, irqdest);
        }
        openpic_write(&OpenPIC->Source[irq].Destination, cpus_addr(physmask)[0]);
}
 ...
 *  sense: 1 for level, 0 for edge
 */
static void openpic_set_sense(struct OpenPIC *OpenPIC, u_int irq, int sense)
{
	openpic_safe_writefield(&OpenPIC->Source[irq].Vector_Priority,
				OPENPIC_SENSE_LEVEL,
...
static void openpic_ack_irq(unsigned int irq_nr)
{
	if (irq_nr >= open_pic2_irq_offset) {
...
static void openpic_set_affinity(unsigned int irq_nr, cpumask_t cpumask)
{
	if ((irq_nr >= open_pic_irq_offset)
	    && (irq_nr < open_pic_irq_offset + NumSources))
		openpic_mapirq((struct OpenPIC __iomem *)OpenPIC,
			       irq_nr - open_pic_irq_offset, physmask(cpumask),
				CPU_MASK_NONE);
	else if ((irq_nr >= open_pic2_irq_offset)
	    && (irq_nr < open_pic2_irq_offset + NumSources))
		openpic_mapirq((struct OpenPIC __iomem *)OpenPIC2,
			       irq_nr - open_pic2_irq_offset, physmask(cpumask),
				CPU_MASK_NONE);
}
...
int openpic_get_irq(void)
{
	int irq = openpic_irq();

	if (cascaded[irq]) {
		irq = (cascaded[irq]) ();
		if (irq == OPENPIC_VEC_SPURIOUS + open_pic2_irq_offset)
			irq = -1;
	}
...
void smp_openpic_message_pass(int target, int msg, unsigned long data, int wait)
{
	...
	case MSG_ALL_BUT_SELF:
		openpic_cause_IPI(msg, 0xffffffff & ~(1 << smp_processor_id()));
		break;
	default:
...
void __pmac openpic_sleep_save_intrs(void)
{
	int i;
	unsigned long flags;
	...
	for (i = 0; i < NumProcessors; i++) {
		save_cpu_task_pri[i] =
		    openpic_read(&OpenPIC->Processor[i].Current_Task_Priority);
	...
	for (i = 0; i < NumSources; i++) {
		save_irq_src_vp[i] =
		    openpic_read(&OpenPIC->Source[i].Vector_Priority)
		    & ~OPENPIC_ACTIVITY;
		save_irq_src_dest[i] =
		    openpic_read(&OpenPIC->Source[i].Destination);
	}
	spin_unlock_irqrestore(&openpic_setup_lock, flags);
}

void __pmac openpic_sleep_restore_intrs(void)
{
	int i;
	unsigned long flags;
	...
	for (i = 0; i < OPENPIC_NUM_IPI; i++)
		openpic_write(&OpenPIC->Global.IPI_Vector_Priority(i),
			      save_ipi_vp[i]);
	for (i = 0; i < NumSources; i++) {
		openpic_write(&OpenPIC->Source[i].Vector_Priority,
			      save_irq_src_vp[i]);
		openpic_write(&OpenPIC->Source[i].Destination,
			      save_irq_src_dest[i]);
	}
